...
const { isDashboardAdmin, sendDashboardPage } = require('./dashboard');
...
function registerCallbacks(bot) {
  bot.on('callback_query', async (query) => {
    ...
    try {
      ...
      } else if (data.startsWith('dashboard_page:')) {
        if (!isDashboardAdmin(userId)) {
          await bot.sendMessage(chatId, '⚠️ غير مصرح لك باستخدام هذا الأمر.');
        } else {
          const requestedPage = parseInt(data.slice('dashboard_page:'.length), 10) || 0;
          await sendDashboardPage(bot, chatId, query.message.message_id, requestedPage);
        }
      }
    } catch (err) {
